/**
 * Semantic Token Constants
 * DO NOT EDIT DIRECTLY - This file is auto-generated
 * Generated on Wed, 09 Sep 2026 21:49:51 GMT
 */

export const Style = {
	BACKGROUND: 'var(--background)',
	FOREGROUND: 'var(--foreground)',
	CARD: 'var(--card)',
	CARD_FOREGROUND: 'var(--card-foreground)',
	POPOVER: 'var(--popover)',
	POPOVER_FOREGROUND: 'var(--popover-foreground)',
	PRIMARY: 'var(--primary)',
	SECONDARY: 'var(--secondary)',
	MUTED: 'var(--muted)',
	MUTED_FOREGROUND: 'var(--muted-foreground)',
	ACCENT: 'var(--accent)',
	ACCENT_FOREGROUND: 'var(--accent-foreground)',
	DESTRUCTIVE: 'var(--destructive)',
	DESTRUCTIVE_FOREGROUND: 'var(--destructive-foreground)',
	BORDER: 'var(--border)',
	INPUT: 'var(--input)',
	RING: 'var(--ring)',
	RADIUS: 'var(--radius)',
	CHART_1: 'var(--chart-1)',
	CHART_2: 'var(--chart-2)',
	CHART_3: 'var(--chart-3)',
	CHART_4: 'var(--chart-4)',
	CHART_5: 'var(--chart-5)',
	SIDEBAR: 'var(--sidebar)',
	SIDEBAR_FOREGROUND: 'var(--sidebar-foreground)',
	SIDEBAR_PRIMARY: 'var(--sidebar-primary)',
	SIDEBAR_PRIMARY_FOREGROUND: 'var(--sidebar-primary-foreground)',
	SIDEBAR_ACCENT: 'var(--sidebar-accent)',
	SIDEBAR_ACCENT_FOREGROUND: 'var(--sidebar-accent-foreground)',
	SIDEBAR_BORDER: 'var(--sidebar-border)',
	SIDEBAR_RING: 'var(--sidebar-ring)',
	FONT_SANS: 'var(--font-sans)',
	FONT_SERIF: 'var(--font-serif)',
	FONT_MONO: 'var(--font-mono)',
	BASE_WHITE: 'var(--base-white)',
	BASE_BLACK: 'var(--base-black)',
	L1_BACKGROUND: 'var(--l1-background)',
	L2_BACKGROUND: 'var(--l2-background)',
	L3_BACKGROUND: 'var(--l3-background)',
	L1_BACKGROUND_HOVER: 'var(--l1-background-hover)',
	L2_BACKGROUND_HOVER: 'var(--l2-background-hover)',
	L3_BACKGROUND_HOVER: 'var(--l3-background-hover)',
	L1_FOREGROUND: 'var(--l1-foreground)',
	L1_FOREGROUND_HOVER: 'var(--l1-foreground-hover)',
	L2_FOREGROUND: 'var(--l2-foreground)',
	L2_FOREGROUND_HOVER: 'var(--l2-foreground-hover)',
	L3_FOREGROUND: 'var(--l3-foreground)',
	L3_FOREGROUND_HOVER: 'var(--l3-foreground-hover)',
	L1_BORDER: 'var(--l1-border)',
	L2_BORDER: 'var(--l2-border)',
	L3_BORDER: 'var(--l3-border)',
	PRIMARY_BACKGROUND: 'var(--primary-background)',
	PRIMARY_BACKGROUND_HOVER: 'var(--primary-background-hover)',
	PRIMARY_FOREGROUND: 'var(--primary-foreground)',
	PRIMARY_FOREGROUND_HOVER: 'var(--primary-foreground-hover)',
	SECONDARY_BACKGROUND: 'var(--secondary-background)',
	SECONDARY_BACKGROUND_HOVER: 'var(--secondary-background-hover)',
	SECONDARY_FOREGROUND: 'var(--secondary-foreground)',
	SECONDARY_FOREGROUND_HOVER: 'var(--secondary-foreground-hover)',
	SECONDARY_BORDER: 'var(--secondary-border)',
	SUCCESS_BACKGROUND: 'var(--success-background)',
	SUCCESS_BACKGROUND_HOVER: 'var(--success-background-hover)',
	SUCCESS_FOREGROUND: 'var(--success-foreground)',
	SUCCESS_FOREGROUND_HOVER: 'var(--success-foreground-hover)',
	WARNING_BACKGROUND: 'var(--warning-background)',
	WARNING_BACKGROUND_HOVER: 'var(--warning-background-hover)',
	WARNING_FOREGROUND: 'var(--warning-foreground)',
	WARNING_FOREGROUND_HOVER: 'var(--warning-foreground-hover)',
	DANGER_BACKGROUND: 'var(--danger-background)',
	DANGER_BACKGROUND_HOVER: 'var(--danger-background-hover)',
	DANGER_FOREGROUND: 'var(--danger-foreground)',
	DANGER_FOREGROUND_HOVER: 'var(--danger-foreground-hover)',
	PRIMARY_LINK: 'var(--primary-link)',
	PRIMARY_LINK_HOVER: 'var(--primary-link-hover)',
	SECONDARY_LINK: 'var(--secondary-link)',
	SECONDARY_LINK_HOVER: 'var(--secondary-link-hover)',
	SUCCESS_LINK: 'var(--success-link)',
	SUCCESS_LINK_HOVER: 'var(--success-link-hover)',
	WARNING_LINK: 'var(--warning-link)',
	WARNING_LINK_HOVER: 'var(--warning-link-hover)',
	DANGER_LINK: 'var(--danger-link)',
	DANGER_LINK_HOVER: 'var(--danger-link-hover)',
	L1_BACKGROUND_TRANSPARENT: 'var(--l1-background-transparent)',
	L1_BACKGROUND_60: 'var(--l1-background-60)',
	L2_BACKGROUND_TRANSPARENT: 'var(--l2-background-transparent)',
	L2_BACKGROUND_60: 'var(--l2-background-60)',
	L3_BACKGROUND_TRANSPARENT: 'var(--l3-background-transparent)',
	L3_BACKGROUND_60: 'var(--l3-background-60)',
	L3_BACKGROUND_30: 'var(--l3-background-30)',
	SURFACE_1: 'var(--surface-1)',
	SURFACE_2: 'var(--surface-2)',
	SURFACE_3: 'var(--surface-3)',
	SURFACE_STATIC: 'var(--surface-static)',
	ALERT_STRIP_BUTTON_BACKGROUND: 'var(--alert-strip-button-background)',
	CALLOUT_PRIMARY_BACKGROUND: 'var(--callout-primary-background)',
	CALLOUT_PRIMARY_BORDER: 'var(--callout-primary-border)',
	CALLOUT_PRIMARY_TITLE: 'var(--callout-primary-title)',
	CALLOUT_PRIMARY_DESCRIPTION: 'var(--callout-primary-description)',
	CALLOUT_PRIMARY_ICON: 'var(--callout-primary-icon)',
	CALLOUT_SUCCESS_BACKGROUND: 'var(--callout-success-background)',
	CALLOUT_SUCCESS_BORDER: 'var(--callout-success-border)',
	CALLOUT_SUCCESS_TITLE: 'var(--callout-success-title)',
	CALLOUT_SUCCESS_DESCRIPTION: 'var(--callout-success-description)',
	CALLOUT_SUCCESS_ICON: 'var(--callout-success-icon)',
	CALLOUT_WARNING_BACKGROUND: 'var(--callout-warning-background)',
	CALLOUT_WARNING_BORDER: 'var(--callout-warning-border)',
	CALLOUT_WARNING_TITLE: 'var(--callout-warning-title)',
	CALLOUT_WARNING_DESCRIPTION: 'var(--callout-warning-description)',
	CALLOUT_WARNING_ICON: 'var(--callout-warning-icon)',
	CALLOUT_ERROR_BACKGROUND: 'var(--callout-error-background)',
	CALLOUT_ERROR_BORDER: 'var(--callout-error-border)',
	CALLOUT_ERROR_TITLE: 'var(--callout-error-title)',
	CALLOUT_ERROR_DESCRIPTION: 'var(--callout-error-description)',
	CALLOUT_ERROR_ICON: 'var(--callout-error-icon)',
	ACCENT_PRIMARY: 'var(--accent-primary)',
	ACCENT_PRIMARY_HOVER: 'var(--accent-primary-hover)',
	ACCENT_PRIMARY_FOREGROUND: 'var(--accent-primary-foreground)',
	ACCENT_FOREST: 'var(--accent-forest)',
	ACCENT_FOREST_HOVER: 'var(--accent-forest-hover)',
	ACCENT_FOREST_FOREGROUND: 'var(--accent-forest-foreground)',
	ACCENT_AMBER: 'var(--accent-amber)',
	ACCENT_AMBER_HOVER: 'var(--accent-amber-hover)',
	ACCENT_AMBER_FOREGROUND: 'var(--accent-amber-foreground)',
	ACCENT_CHERRY: 'var(--accent-cherry)',
	ACCENT_CHERRY_HOVER: 'var(--accent-cherry-hover)',
	ACCENT_CHERRY_FOREGROUND: 'var(--accent-cherry-foreground)',
	ACCENT_AQUA: 'var(--accent-aqua)',
	ACCENT_AQUA_HOVER: 'var(--accent-aqua-hover)',
	ACCENT_AQUA_FOREGROUND: 'var(--accent-aqua-foreground)',
	ACCENT_SAKURA: 'var(--accent-sakura)',
	ACCENT_SAKURA_HOVER: 'var(--accent-sakura-hover)',
	ACCENT_SAKURA_FOREGROUND: 'var(--accent-sakura-foreground)',
	ACCENT_SIENNA: 'var(--accent-sienna)',
	ACCENT_SIENNA_HOVER: 'var(--accent-sienna-hover)',
	ACCENT_SIENNA_FOREGROUND: 'var(--accent-sienna-foreground)',
	GHOST_BACKGROUND: 'var(--ghost-background)',
	GHOST_BACKGROUND_HOVER: 'var(--ghost-background-hover)',
	GHOST_FOREGROUND: 'var(--ghost-foreground)',
	GHOST_FOREGROUND_HOVER: 'var(--ghost-foreground-hover)',
	GHOST_BORDER: 'var(--ghost-border)',
	ACTION_BACKGROUND: 'var(--action-background)',
	ACTION_BACKGROUND_HOVER: 'var(--action-background-hover)',
	ACTION_BORDER: 'var(--action-border)',
	ACTION_BORDER_HOVER: 'var(--action-border-hover)',
	ACTION_FOREGROUND: 'var(--action-foreground)',
	ACTION_FOREGROUND_HOVER: 'var(--action-foreground-hover)',
	CALLOUT_SIENNA_BACKGROUND: 'var(--callout-sienna-background)',
	CALLOUT_SIENNA_BORDER: 'var(--callout-sienna-border)',
	CALLOUT_SIENNA_TITLE: 'var(--callout-sienna-title)',
	CALLOUT_SIENNA_DESCRIPTION: 'var(--callout-sienna-description)',
	CALLOUT_SIENNA_ICON: 'var(--callout-sienna-icon)',
	CALLOUT_AQUA_BACKGROUND: 'var(--callout-aqua-background)',
	CALLOUT_AQUA_BORDER: 'var(--callout-aqua-border)',
	CALLOUT_AQUA_TITLE: 'var(--callout-aqua-title)',
	CALLOUT_AQUA_DESCRIPTION: 'var(--callout-aqua-description)',
	CALLOUT_AQUA_ICON: 'var(--callout-aqua-icon)',
	CONTROL_ICON_FOREGROUND: 'var(--control-icon-foreground)',
	CONTROL_ICON_FOREGROUND_CONTRAST: 'var(--control-icon-foreground-contrast)',
	SHADOW_SIDEBAR: 'var(--shadow-sidebar)',
	SHADOW_DROPDOWN: 'var(--shadow-dropdown)',
	SHADOW_DRAWER: 'var(--shadow-drawer)',
	SHADOW_DIALOG: 'var(--shadow-dialog)',
	SHADOW_TOAST: 'var(--shadow-toast)',
	SHADOW_TOOLTIP: 'var(--shadow-tooltip)',
	PILL_PRIMARY_BACKGROUND: 'var(--pill-primary-background)',
	PILL_PRIMARY_BACKGROUND_HOVER: 'var(--pill-primary-background-hover)',
	PILL_PRIMARY_BORDER: 'var(--pill-primary-border)',
	PILL_PRIMARY_BORDER_HOVER: 'var(--pill-primary-border-hover)',
	PILL_PRIMARY_LABEL: 'var(--pill-primary-label)',
	PILL_PRIMARY_LABEL_HOVER: 'var(--pill-primary-label-hover)',
	PILL_PRIMARY_ICON: 'var(--pill-primary-icon)',
	PILL_SUCCESS_BACKGROUND: 'var(--pill-success-background)',
	PILL_SUCCESS_BACKGROUND_HOVER: 'var(--pill-success-background-hover)',
	PILL_SUCCESS_BORDER: 'var(--pill-success-border)',
	PILL_SUCCESS_BORDER_HOVER: 'var(--pill-success-border-hover)',
	PILL_SUCCESS_LABEL: 'var(--pill-success-label)',
	PILL_SUCCESS_LABEL_HOVER: 'var(--pill-success-label-hover)',
	PILL_SUCCESS_ICON: 'var(--pill-success-icon)',
	PILL_WARNING_BACKGROUND: 'var(--pill-warning-background)',
	PILL_WARNING_BACKGROUND_HOVER: 'var(--pill-warning-background-hover)',
	PILL_WARNING_BORDER: 'var(--pill-warning-border)',
	PILL_WARNING_BORDER_HOVER: 'var(--pill-warning-border-hover)',
	PILL_WARNING_LABEL: 'var(--pill-warning-label)',
	PILL_WARNING_LABEL_HOVER: 'var(--pill-warning-label-hover)',
	PILL_WARNING_ICON: 'var(--pill-warning-icon)',
	PILL_DANGER_BACKGROUND: 'var(--pill-danger-background)',
	PILL_DANGER_BACKGROUND_HOVER: 'var(--pill-danger-background-hover)',
	PILL_DANGER_BORDER: 'var(--pill-danger-border)',
	PILL_DANGER_BORDER_HOVER: 'var(--pill-danger-border-hover)',
	PILL_DANGER_LABEL: 'var(--pill-danger-label)',
	PILL_DANGER_LABEL_HOVER: 'var(--pill-danger-label-hover)',
	PILL_DANGER_ICON: 'var(--pill-danger-icon)',
	PILL_FILLED_BACKGROUND: 'var(--pill-filled-background)',
	PILL_FILLED_BACKGROUND_HOVER: 'var(--pill-filled-background-hover)',
	PILL_FILLED_BORDER: 'var(--pill-filled-border)',
	PILL_FILLED_BORDER_HOVER: 'var(--pill-filled-border-hover)',
	PILL_FILLED_LABEL: 'var(--pill-filled-label)',
	PILL_FILLED_LABEL_HOVER: 'var(--pill-filled-label-hover)',
	PILL_FILLED_ICON: 'var(--pill-filled-icon)',
	PILL_OUTLINED_BACKGROUND: 'var(--pill-outlined-background)',
	PILL_OUTLINED_BACKGROUND_HOVER: 'var(--pill-outlined-background-hover)',
	PILL_OUTLINED_BORDER: 'var(--pill-outlined-border)',
	PILL_OUTLINED_BORDER_HOVER: 'var(--pill-outlined-border-hover)',
	PILL_OUTLINED_LABEL: 'var(--pill-outlined-label)',
	PILL_OUTLINED_LABEL_HOVER: 'var(--pill-outlined-label-hover)',
	PILL_OUTLINED_ICON: 'var(--pill-outlined-icon)',
	PILL_PRIMARY_ICON_HOVER: 'var(--pill-primary-icon-hover)',
	PILL_SUCCESS_ICON_HOVER: 'var(--pill-success-icon-hover)',
	PILL_WARNING_ICON_HOVER: 'var(--pill-warning-icon-hover)',
	PILL_DANGER_ICON_HOVER: 'var(--pill-danger-icon-hover)',
	PILL_FILLED_ICON_HOVER: 'var(--pill-filled-icon-hover)',
	PILL_OUTLINED_ICON_HOVER: 'var(--pill-outlined-icon-hover)',
	PILL_RECT_BACKGROUND: 'var(--pill-rect-background)',
	PILL_RECT_BACKGROUND_HOVER: 'var(--pill-rect-background-hover)',
	PILL_RECT_BORDER: 'var(--pill-rect-border)',
	PILL_RECT_BORDER_HOVER: 'var(--pill-rect-border-hover)',
	PILL_RECT_LABEL: 'var(--pill-rect-label)',
	PILL_RECT_LABEL_HOVER: 'var(--pill-rect-label-hover)',
	PILL_RECT_ICON: 'var(--pill-rect-icon)',
	PILL_RECT_ICON_HOVER: 'var(--pill-rect-icon-hover)',
	PILL_RECT_DISMISS_SCRIM_FROM: 'var(--pill-rect-dismiss-scrim-from)',
	PILL_RECT_DISMISS_SCRIM_TO: 'var(--pill-rect-dismiss-scrim-to)',
	PILL_RECT_DISMISS_ICON: 'var(--pill-rect-dismiss-icon)',
	PILL_RECT_SOLID_BACKGROUND: 'var(--pill-rect-solid-background)',
	PILL_RECT_SOLID_BACKGROUND_HOVER: 'var(--pill-rect-solid-background-hover)',
	PILL_RECT_SOLID_LABEL: 'var(--pill-rect-solid-label)',
	PILL_RECT_SOLID_LABEL_HOVER: 'var(--pill-rect-solid-label-hover)',
	PILL_RECT_SOLID_ICON: 'var(--pill-rect-solid-icon)',
	PILL_RECT_SOLID_ICON_HOVER: 'var(--pill-rect-solid-icon-hover)',
	PILL_RECT_SOLID_DISMISS_ICON: 'var(--pill-rect-solid-dismiss-icon)',
	PILL_RECT_SOLID_DISMISS_ICON_HOVER:
		'var(--pill-rect-solid-dismiss-icon-hover)',
	PILL_PRIMARY_DOT: 'var(--pill-primary-dot)',
	PILL_SUCCESS_DOT: 'var(--pill-success-dot)',
	PILL_WARNING_DOT: 'var(--pill-warning-dot)',
	PILL_DANGER_DOT: 'var(--pill-danger-dot)',
	PILL_FILLED_DOT: 'var(--pill-filled-dot)',
	PILL_OUTLINED_DOT: 'var(--pill-outlined-dot)',
	PILL_RECT_DOT: 'var(--pill-rect-dot)',
	PILL_RECT_SOLID_DOT: 'var(--pill-rect-solid-dot)',
	BADGE_PRIMARY_OUTLINED_BACKGROUND: 'var(--badge-primary-outlined-background)',
	BADGE_PRIMARY_OUTLINED_BORDER: 'var(--badge-primary-outlined-border)',
	BADGE_PRIMARY_OUTLINED_LABEL: 'var(--badge-primary-outlined-label)',
	BADGE_PRIMARY_OUTLINED_ICON: 'var(--badge-primary-outlined-icon)',
	BADGE_SUCCESS_OUTLINED_BACKGROUND: 'var(--badge-success-outlined-background)',
	BADGE_SUCCESS_OUTLINED_BORDER: 'var(--badge-success-outlined-border)',
	BADGE_SUCCESS_OUTLINED_LABEL: 'var(--badge-success-outlined-label)',
	BADGE_SUCCESS_OUTLINED_ICON: 'var(--badge-success-outlined-icon)',
	BADGE_WARNING_OUTLINED_BACKGROUND: 'var(--badge-warning-outlined-background)',
	BADGE_WARNING_OUTLINED_BORDER: 'var(--badge-warning-outlined-border)',
	BADGE_WARNING_OUTLINED_LABEL: 'var(--badge-warning-outlined-label)',
	BADGE_WARNING_OUTLINED_ICON: 'var(--badge-warning-outlined-icon)',
	BADGE_DANGER_OUTLINED_BACKGROUND: 'var(--badge-danger-outlined-background)',
	BADGE_DANGER_OUTLINED_BORDER: 'var(--badge-danger-outlined-border)',
	BADGE_DANGER_OUTLINED_LABEL: 'var(--badge-danger-outlined-label)',
	BADGE_DANGER_OUTLINED_ICON: 'var(--badge-danger-outlined-icon)',
	BADGE_SECONDARY_OUTLINED_BACKGROUND:
		'var(--badge-secondary-outlined-background)',
	BADGE_SECONDARY_OUTLINED_BORDER: 'var(--badge-secondary-outlined-border)',
	BADGE_SECONDARY_OUTLINED_LABEL: 'var(--badge-secondary-outlined-label)',
	BADGE_SECONDARY_OUTLINED_ICON: 'var(--badge-secondary-outlined-icon)',
} as const;

export type StyleType = typeof Style;
